...
- **Three faces, loaded from Google Fonts via `next/font`:**
  - **Display: Newsreader** (serif), weight 500 for headings and 600 for the big stat numbers.
    Used for: the name in the hero and top bar, case-study titles, project card titles,
    company names, school names, award titles, decision card titles, persona names and
    pull quotes. Newsreader has a smaller apparent size than Inter, so display sizes are
    bumped roughly 10% and letter-spacing is loosened to about -.012em (Inter needed -.025em).
  - **Body: Inter** for all reading text, bullets, card descriptions and narratives.
  - **Mono: JetBrains Mono** for every label, date, caption, tag, chip, section heading,
    table header and the left rail.
  Take the exact per-element sizes from the mockup's `--display` usages. Getting the serif
  onto the right elements and nowhere else is the whole look.
...
**Education row** — 40px logo tile, school and degree on the left; on the right, a
right-aligned stack with the location above the dates, both in mono (location slightly
darker than the dates).
